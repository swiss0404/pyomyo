...
def draw(w, nx, ny, nz):
    ...
    if error_str != '':
        drawTextwithColor((-2.6, -2, 2), error_str + ', Press any key to clear error', 16, color=(255,0,0))
    else:
        if is_calibrated:
            if confirm_prompt:
                drawTextwithColor((-2.6, -2, 2), f"REVERT BACK to {pre_add_rep-1} CONFIRM? press 'y' to confirm any to abort", 16,color=(255,0,0))
            else:
                drawText((-2.6, -1.8, 2), "'s' to start, 'p' to pause, 'q' to save and exit ", 16)
                drawText((-2.6, -2, 2), "'n' to save and next, 'd' to drop this rep", 16)
            if is_start:
                if is_recording:
                    drawText((-2.6, 1.6, 2), "RECORDING 'p' to pause", 18)
                    drawTextwithColor((-2.6, 1.4, 2), f"RECORDING {gesture} {pre_add_rep+1}", 16, color=(0,255,0))
                else:
            ...
            drawTextwithColor((-2.6, 1.6, 2), "press 'c' to calibrate", 18,color=(255,0,0))
            drawText((-2.6, -1.8, 2), "Please calibrate first press 'c' to calibrate ", 16)
            if confirm_prompt:
                drawTextwithColor((-2.6, -2, 2), f"REVERT BACK to {pre_add_rep-1} CONFIRM? press 'y' to confirm any to abort", 16,color=(255,0,0))
            else:
                            ...
                            if error_str == '':
                                if not is_calibrated:
                                    ...
                                    if ev.unicode == 'e':
                                        ...
                                        if record_cache_imu != [] and record_cache_emg != []:
                                        ...
                                        if record_cache_imu != [] and record_cache_emg != []:
                                            ...
                                            pickle.dump(record_cache_imu, open('examples/data/'+ name + "_" + gesture + "_" + str(pre_add_rep+1) + "_" + "imu_rec.p", "wb" ) )
                                            pre_add_rep += 1
                                            data.loc[(data.name == name) & (data.gesture == gesture),['repetition']] = pre_add_rep
                                            data.to_csv(open(database_file, "wb" ), index = False)
                                            is_start = False
                                            is_calibrated = False
                                            is_recording = False
