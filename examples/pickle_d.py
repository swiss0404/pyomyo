...
emg_file = 'examples/data/song_fist_1_emg_rec.p'
imu_file = 'examples/data/song_fist_1_imu_rec.p'
...
emg_ticks = [(tick - emg_ticks[0])/1000 for tick in emg_ticks] #normalize emg_ticks and change milisec into sec
...
for i in subplot_list_emg:
    lines = [((x0,y0), (x1,y1)) for x0, y0, x1, y1 in zip(emg_ticks[:-1], df_emg[channel][:-1], emg_ticks[1:], df_emg[channel][1:])]
...
for i in subplot_list_imu:
    lines = [((x0,y0), (x1,y1)) for x0, y0, x1, y1 in zip(imu_ticks[:-1], df_imu[channel][:-1], imu_ticks[1:], df_imu[channel][1:])]
    colored_lines = LineCollection(lines, colors=c, linewidths=(1,))
    i.add_collection(colored_lines)
    i.autoscale_view()
...
save_df_imu = pd.DataFrame(imu, columns=['raw_imu','adjusted_ypr','neutral_ypr','is_recording','emg_ticks'])
